...
function parseTagImpl(str: string) {
...
function parseType(str: string, data: { errors?: [] }) {
...
function parseName(str: string, data: { errors?: [] }) {
  if (data.errors && data.errors.length) {
    return null
  }
  ...
  let name = ''
  let brackets = 0
  let res = { optional: false, name: undefined, default: undefined }
  ...
  if (
    ...
    res = { name: name, optional: false, default: undefined }
...
function parseDescription(str: string, data: { errors?: [] }) {
  if (data.errors && data.errors.length) {
    return null
  }
...
function parseTag(
  str: string,
  parsers: ((source: string, data: any) => void)[],
) {
...
function parseBlock(
  ...
  let trim = opts.trim ? (s: string) => s.trim() : (s: string): string => s

  let toggleFence =
    typeof opts.fence === 'function'
      ? opts.fence
      : (line: string) => line.split(opts.fence).length % 2 === 0

  let sourceStr = source
    .map((line: { source: string }) => {
  ...
  let tags = source.reduce(function (tags, tag) {
    let tagNode = parseTag(tag.source, opts.parsers) as any
    if (!tagNode) {
      return tags
    }
